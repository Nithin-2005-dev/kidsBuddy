...
const MoralsTellingPage = () => {
  ...
  const categories = ['Honesty', ' Respect', 'Responsibility', 'Fairness'];
  ...
  const handleCategorySelect = async (category) => {
    ...
    try {
        const response = await askApi(`tell the moral values on category ${category} for children in ${languageRef.current.value==''?'English':languageRef.current.value} language`);
        setStory(response)
        setIsLoading(false)
      } catch (error) {
  ...
  return (
    ...
    <hr className='my-2'></hr>
      <h1 className="text-3xl font-bold mb-8">Choose a Moral Value</h1>
      ...
      {isLoading ? (
        <div className="text-center text-xl">Loading Moral Value...</div>
      ) : error ? (
        <div className="text-center text-xl text-red-600">{error}</div>
      ) : story ? (
        <div className="bg-opacity-80 p-6 rounded-lg shadow-md">
          <h2 className="text-2xl font-semibold mb-4">{selectedCategory} Moral Values</h2>
...
export default MoralsTellingPage;
